...
let ShieldService = class ShieldService {
    ...
    decodeNestedUnicode(text) {
        // Decode \u0027, \u005c, etc.
        return text.replace(/\\u([0-9a-fA-F]{4})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
    }
    normalizeSqlComments(text) {
    ...
    detectObfuscation(text) {
        ...
        if ((text.match(/\\u[0-9a-fA-F]{4}/g) || []).length > 2)
            score += 15;
        // Null bytes
        if (/\x00/.test(text))
            score += 20;
        // Mixed case SQL keywords with comments between chars
        if (/s\/\*.*?\*\/e\/\*.*?\*\/l\/\*.*?\*\/e\/\*.*?\*\/c\/\*.*?\*\/t/i.test(text))
            score += 25;
        return score;
    }
};
exports.ShieldService = ShieldService;
exports.ShieldService = ShieldService = __decorate([
    (0, common_1.Injectable)()
], ShieldService);
//# sourceMappingURL=shield.service.js.map